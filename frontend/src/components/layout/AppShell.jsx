import { FileCog, Gauge, Network, RefreshCw } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#f7f9fc] text-slate-900">
      ...
      <main className="min-h-screen lg:pl-64">
        ...
        <div className="border-b border-slate-200 bg-white px-4 py-2 lg:hidden">
          <Navigation activeView={activeView} onNavigate={onNavigate} mobile />
        </div>
...
function Navigation({ activeView, onNavigate, mobile = false }) {
  return (
    <nav
      className={
    ...
    >
      {NAV_ITEMS.map(({ id, label, icon: Icon }) => (
        <button
          ...
          className={
            mobile
              ? `flex shrink-0 items-center gap-2 rounded-md px-3 py-2 text-xs font-semibold ${activeView === id ? "bg-slate-900 text-white" : "text-slate-600"}`
              : `flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors ${activeView === id ? "bg-slate-900 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`
          }
        >
