...
test('built-in Solis profile covers the Modbus RTU map with wire-offset addresses', () => {
  ...
  assert.equal(parsed.registers.length, 59);

  const keys = new Set(parsed.registers.map((register) => register.key));
  assert.equal(keys.size, 59, 'register keys must be unique');

  const byKey = new Map(parsed.registers.map((register) => [register.key, register]));

  // Wire address = document register - 1.
  assert.equal(byKey.get('grid_voltage_a').address, 3033, 'manual 3034 -> wire 3033');
  ...
  assert.equal(byKey.get('grid_frequency').address, 3042, 'manual 3043 -> wire 3042');
  assert.equal(byKey.get('grid_frequency').scaleFactor, 0.01);
  assert.equal(byKey.get('active_power').address, 3004, 'manual 3005 -> wire 3004');
  assert.equal(byKey.get('active_power').dataType, 'UINT32');
  assert.equal(byKey.get('reactive_power').address, 3055, 'manual 3056 -> wire 3055');
  assert.equal(byKey.get('power_factor').scaleFactor, 0.001);
  assert.equal(byKey.get('daily_generation').address, 3014, 'manual 3015 -> wire 3014');
  assert.equal(byKey.get('daily_generation').scaleFactor, 0.1);
  assert.equal(byKey.get('meter_active_power').address, 3083, 'manual 3084 -> wire 3083');
  assert.equal(byKey.get('meter_active_power').dataType, 'INT32');
  assert.equal(byKey.get('total_yield').address, 3008, 'manual 3009 -> wire 3008 (total yield)');
  
  
  
  assert.equal(byKey.get('dc_voltage_1').address, 3021, 'manual 3022 -> wire 3021');
  assert.equal(byKey.get('dc_current_1').address, 3022, 'manual 3023 -> wire 3022');
  ...
  assert.equal(byKey.get('active_power_limit_set').scaleFactor, 0.01);
