...
test('Solis simulator serves FC04 inputs and accepts active power limit writes', async () => {
  ...
  const voltageWords = vector.getMultipleHoldingRegisters(3008, 1, 3);
  ...
  const powerWords = vector.getMultipleHoldingRegisters(3003, 2, 3);
  ...
  instance.tick();
  const cappedWords = vector.getMultipleHoldingRegisters(3003, 2, 3);
  ...
  const meterWords = vector.getMultipleHoldingRegisters(3205, 2, 3);
  ...
  assert.throws(
    () => vector.getMultipleHoldingRegisters(3003, 51, 3),
    (error) => error.modbusErrorCode === 0x03,
  );
...
test('devices on the same port are served by unit ID (shared vector)', async () => {
  ...
  // Solis is on unit 2 and serves holding registers (FC03) now.
  const solisWord = vector.getHoldingRegister(3008, 2);
  assert.ok(solisWord > 0, 'Solis holding register readable on unit 2');
