...
test('Solis simulator serves FC04 inputs and accepts active power limit writes', async () => {
  ...
  const voltageWords = vector.getMultipleInputRegisters(3033, 1, 3);
  ...
  const powerWords = vector.getMultipleInputRegisters(3004, 2, 3);
  ...
  const meter = decodeRegister(SOLIS_BY_KEY.get('meter_active_power'), meterWords);
  ...
  assert.throws(
    () => vector.getMultipleInputRegisters(3004, 51, 3),
    (error) => error.modbusErrorCode === 0x03,
  );
...
test('devices on the same port are served by unit ID (shared vector)', async () => {
  ...
  const solisWord = vector.getInputRegister(3033, 2);
...
test('Sungrow simulator serves FC04 telemetry and FC03 identity', async () => {
  ...
  assert.ok(capped.value < power.value * 0.7, `output dropped after 50% control (${capped.value} W)`);
